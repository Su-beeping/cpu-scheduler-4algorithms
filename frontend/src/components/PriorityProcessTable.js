...
export default function PriorityProcessTable({ processes, onUpdate, onRemove, onAdd, onSimulate, onReset, loading, error }) {
  return (
    <section className="card input-card">
      <div className="card-header">
        <h2>Processes</h2>
        <p className="hint-text">Lower priority number = runs first</p>
      </div>
          ...
          <tbody>
            {processes.map((p, i) => (
              <tr key={p.id}>
                ...
                <td>
                  <input className="cell-input" type="number" min="1" value={p.priority}
